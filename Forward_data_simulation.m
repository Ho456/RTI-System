%% Description
%{
Description: Simulates Forward 2D MoM case without conjugate gradient approach. Program is benchmarked with SOM 
...
imp = 120*pi;
geom = 'square';
NodeMode=1; % nodes are transreceivers
NaNremove=1;
%% Method of Moment
...
Obj_loc_x = 0.12;
Obj_loc_y = 0.3;
epsono_r((x+Obj_loc_x).^2+(y-Obj_loc_y).^2<=(obj_rad)^2) = epsono_r_c;
%%
...
if NodeMode==1  % nodes are transreceivers
TxRxpairs=[];
for iii=1:length(Tx_pos(:,1))
    for jjj=1:length(Rx_pos(:,1))
        if iii~=jjj
